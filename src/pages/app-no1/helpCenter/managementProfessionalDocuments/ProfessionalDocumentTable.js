...
import { Badge, Dropdown, DropdownItem, DropdownMenu, DropdownToggle, Table } from 'reactstrap'
...
import { handleConvertTOISOstring, TLNV, USER_ROLE } from '../utility'
import { APPROVALED, DISABLED, DRAFT, REJECTED, statusTypeFilter, TdStyled, WAITTING_APPROVAL } from './utility'

const ProfessionalDocumentTable = (reActiveLoadApi ) => {
  ...
  const categorySugesstionFilter =!isArrayEmpty(categoryQuestion) ?  categoryQuestion.filter((cate) => {
    return cate.categoryQuestionType === TLNV
  }) : []
    ...
    switch (approvalStatus) {
      case WAITTING_APPROVAL:
        return {
          component: (
            <Badge color='warning'>
              <FormattedMessage id={getKeyLang('helpcenter.management.status.waitingApproval')} />
            </Badge>
          ),
          content: intl.formatMessage({ id: getKeyLang(`helpcenter.management.status.waitingApproval`) })
        }
      case APPROVALED:
        return {
          component: (
            <Badge color='success'>
              <FormattedMessage id={getKeyLang('helpcenter.management.status.approvaled')} />
            </Badge>
          ),
          content: intl.formatMessage({ id: getKeyLang(`helpcenter.management.status.approvaled`) })
        }
      case REJECTED:
        return {
          component: (
            <Badge color='danger'>
              <FormattedMessage id={getKeyLang('helpcenter.management.status.rejected')} />
            </Badge>
          ),
          content: intl.formatMessage({ id: getKeyLang(`helpcenter.management.status.rejected`) })
        }
      case DISABLED:
        return {
          component: (
            <Badge color='warning'>
              <FormattedMessage id={getKeyLang('helpcenter.management.status.disabled')} />
            </Badge>
          ),
          content: intl.formatMessage({ id: getKeyLang(`helpcenter.management.status.disabled`) })
        }
      case DRAFT:
        return {
          component: (
            <Badge color='secondary'>
              <FormattedMessage id={getKeyLang('helpcenter.management.status.draft')} />
            </Badge>
          ),
  ...
  const getCategory = (cateId) =>{
    ...
    return !isArrayEmpty(categoryQuestion) ?  cate[0].name : ""
  }
  useEffect(() => {
        ...
        <Table size='sm' responsive bordered>
          <tbody>
            ...
            <tr>
              ...
              <td className='text-left'>{handleConvertTOISOstring(data.createdDate)}</td>
            </tr>
            <tr>
              <TdStyled className='text-left'>
                <FormattedMessage
              ...
              <td className='text-left'>{data.updateDate === null ? '-' : handleConvertTOISOstring(data.updateDate)}</td>
            </tr>
          </tbody>
        </Table>
      ]
    ...
    columns: [
      {
        Header: (
          <FormattedMessage
            id={getKeyLang('helpcenter.management.documentCode')}
          />
        ),
        accessor: 'code',
        filterable: true,
...
export default ProfessionalDocumentTable
