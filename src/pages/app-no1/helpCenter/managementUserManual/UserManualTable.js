...
import { handleConvertTOISOstring, HDSD, USER_ROLE } from '../utility';
...
`
const UserManualTable = (reActiveLoadApi) => {
  ...
  const categorySugesstionFilter = !isArrayEmpty(categoryQuestion) ?  categoryQuestion.filter((cate) => {
      return cate.categoryQuestionType === HDSD
  }) : []
  ...
  return {
    ...
    columns: [
      ...
      },
      {
        ...
        filterMethod : TreeTable_filterMethod_noAccents,
        Cell : ({original}) =>{
          return <span>{original.question}</span>
        }
      },
      {
        Header: (
