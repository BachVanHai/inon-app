...
import FeeTravelDomesticManageBSHView from './FeeTravelDomesticManageBSHView';
import { HttpClient } from 'base-app';
import { useSelector } from 'react-redux';

const FeeTravelDomesticManageView = ({ location }) => {
    ...
    <Row>
      <Col sm="12">
        <Nav tabs>
          ...
          <NavItem>
                <NavLink className={"active"}>PVI</NavLink>
          </NavItem>
        </Nav>
        <TabContent className="py-50" activeTab={companySelected}>
          <FeeTravelDomesticManageBSHView companyId={companySelected} role={role} editable={editable} data={role === 'VIEW' ? location.state.data : ''} userId={userId} childUserArr={childUserArr} />
        </TabContent>
      </Col>
    </Row>
  )
}
export default FeeTravelDomesticManageView
