...
import {
  ...
  getDefault_updateBeneficiariesObj,
  updateBeneficiaries
} from './utility'
      ...
      dispatch(
        updateBeneficiaries(
          getDefault_updateBeneficiariesObj(
            ...
            contractInfo?.beneficiaries[0]?.id,
            contractInfo?.transportationDTOS[0]?.id,
            contractInfo?.cdtyInfoDTO?.id
          )
        )
      )
    return
    }
    dispatch(createContract(getDefault_createContractObj(values)))
  }
    ...
    <StepBase titleMsg={getStepComponent(activeStep).title}>
      <CardBody>
        <FormikProvider value={formik}>
          <StepForm formik={formik} stepInfo={{ step_1 }} />
        </FormikProvider>
      </CardBody>
