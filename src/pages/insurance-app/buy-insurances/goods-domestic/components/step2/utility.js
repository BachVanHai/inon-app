...
import { BASE, KEY_STEP_2 } from '../../../../../../redux/reducers/insurance-app/buyInsurancesGoods'
import { KEY_ADDONS } from '../../../../../../redux/reducers/insurance-app/buyInsurancesGoods'
import { KEY_ACTIVE_STEP, KEY_DATA_FEES } from '../../../../../../redux/reducers/insurance-app/utility'
import { removeCommaBetweenStringAndConvertToNumber } from '../step1/utility'
import { GOODS_TYPE, REDUX_STATE_NAME } from '../stepsManager'
import {
...
export const API_UPDATE_INSURANCES = 'nth/homeinsurance/api/cargo-insurance'
export const API_GET_FEE_INSURANCES =
  'nth/homeinsurance/api/contract-cargo/fee-insurance'
    ...
    if (insurancesRes.status === 200) {
        ...
        const newData = resFeeInsurances.data.map((_elt) => {
          return {
            ..._elt,
            companyId: '01',
            logo: ARR_COMPANIES[0].image,
            totalFeeInsurance: _elt.totalFee,
            nameDetail: ARR_COMPANIES[0].nameDetail
          }
        })
        dispatch(
          updateProps([
...
export const getObj_UpdateInsurance = (values, id, contractId, step_1 , addOns) => {
  const { goodsType, vehicels } = step_1
  const {
    startValueDate,
    endValueDate,
    insuranceMoney,
  ...
  return {
    id: id,
    "productCode": vehicels[0]?.vehicelType,
    "packageCode": goodsType,
    "insuranceCode": GOODS_TYPE,
    "startedDate": moment(startValueDate).utc(true).toISOString(),
    "endDate": moment(endValueDate).utc(true).toISOString(),
    "contractId": contractId,
    "fromPoint": startPoint,
    "endPoint": endPoint,
    "value": removeCommaBetweenStringAndConvertToNumber(insuranceMoney),
    "deduct": removeCommaBetweenStringAndConvertToNumber(insuranceDeduction),
...
export const PERSON_INSURANCE_OPTIONS = [
  ...
  {
    value: OPTS3,
    mgs: 'Biểu diễn nguy hiểm, đua xe, đua ngựa, đua thuyền, thi đấu có tính chất chuyên nghiệp các môn: bóng đá, đấm bốc, leo núi, lướt ván,…',
    subMgs: '(Có có phụ phí bảo hiểm)',
    detail: '',
    fieldName: KEY_PERSON_INSURANCE_TYPE
  }
