...
import { DURATION_PACKAGE, subStringMoney } from './utility'
...
const StepForm = ({ stepInfo, className , packages }) => {
  ...
  const totalFee = dataFees.find(_elt => _elt.companyId === companyId)?.feeInsurances?.totalFeeInsurance
  const responsive = {
    desktop: {
            ...
            >
              <Radio
                ...
                onChange={() => {
                  ...
                  setFieldValue(KEY_END_INSURANCE , moment(getFieldMeta(KEY_START_INSURANCE).value).utc(true).add(_elt.year,'y').format('YYYY-MM-DD'))
                  }                 
                }
              />
