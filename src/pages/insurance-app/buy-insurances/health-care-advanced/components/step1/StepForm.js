...
import {
  ...
  KEY_FULLNAME,
  KEY_GENDER,
  KEY_HAVE_BOUGHT_FOR_ME,
  KEY_IC_NO,
  KEY_IC_NO_BENEFICIARY,
  KEY_IC_TYPE,
  KEY_IC_TYPE_BENEFICIARY,
  KEY_NAME_BENEFICIARY,
  KEY_PHONE_NUMBER,
  KEY_RELATIONSHIP,
  KEY_STK,
  KEY_WARD,
...
const StepForm = ({ stepInfo, className, formik }) => {
  ...
  const postFileCallback = (exelData) => {
    ...
    const convert = (val, type = KEY_DATE_BIRTH) => {
      ...
      if (type === KEY_GENDER) {
        ...
        return val.toString().match(/\w*nam\w*/gi)
          ? sugg_gender[0].content
          : sugg_gender[1].content
      }
      return ''
    }
  ...
  const _checkinfoContact = async () => {
    ...
    if (hasRequestFail(res.status)) {
      BaseAppUltils.toastError(
        intl.formatMessage({ id: getKeyLang(`alert.notExistIcNo`) })
      )
      return
    }
  ...
  return (
    <div className={className}>
      ...
      <TitleRow
        orderNumber='1'
        msg={<FormattedMessage id={getKeyLang(`insuranceBuyerInfo`)} />}
        className='mt-1'
      />
          ...
          {React.useMemo(() => {
            return (
              <Select
                ...
                defaultValue={_buyerIDTypes.find(
                  (type) => type.content === getFieldMeta(KEY_IC_TYPE).value
                )}
              />
            )
            ...
            <Col xs={12} md={4}>
              <Select
                ...
                value={sugg_gender.find(
                  (type) => type.content === getFieldMeta(KEY_GENDER).value
                )}
            ...
            <Col
              ...
              onClick={() => {
                ...
                if (values[KEY_HAVE_BOUGHT_FOR_ME]) {
                  ...
                  _values[KEY_ADDTIONAL_PEOPLE][0] = {
                    ...addtionalPeopleInitValue
                  }
                  setValues(_values)
                  return
                ...
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_IC_TYPE] =
                  values[KEY_IC_TYPE]
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_IC_NO] = values[KEY_IC_NO]
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_FULLNAME] =
                  values[KEY_FULLNAME]
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_DATE_BIRTH] =
                  values[KEY_DATE_BIRTH]
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_PHONE_NUMBER] =
                ...
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_DISTRICT] =
                  values[KEY_DISTRICT]
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_WARD] = values[KEY_WARD]
                _values[KEY_ADDTIONAL_PEOPLE][0][KEY_ADDRESS] =
                  values[KEY_ADDRESS]
                setValues(_values)
              }}
            >
      ...
      <Row className='mb-2'>
        <Col xs={12} md={7} className='mb-1'>
          <div className='mb-1 mt-1'>
            (*) Độ tuổi tham gia từ 60 ngày đến 65 tuổi
          </div>
        </Col>
      </Row>
      ...
      <Row >
        <Col md={4} xs={12}>
          ...
          {React.useMemo(() => {
            return (
              <BaseFormGroup
              fieldName={KEY_NAME_BENEFICIARY}
              errors={errors}
              touched={touched}
              messageId={getKeyLang(`Name`)}
            />
            ...
            getFieldMeta(KEY_NAME_BENEFICIARY).value,
            getFieldMeta(KEY_NAME_BENEFICIARY).error
          ])}
        ...
        {React.useMemo(() => {
            return (
              <Select
                ...
                className='custom-zindex9'
                onChange={({ content }) => {
                  setFieldValue(KEY_IC_TYPE_BENEFICIARY, content)
                }}
                value={_buyerIDTypes.find(
                  (type) => type.content === getFieldMeta(KEY_IC_TYPE_BENEFICIARY).value
                )}
                options={_buyerIDTypes}
                placeholder={<FormattedMessage id={getKeyLang(`IDType`)} />}
                isClearable={false}
                maxMenuHeight={120}
                styles={
                  getFieldMeta(KEY_IC_TYPE_BENEFICIARY).error
                    ? selectErrorStyles
                    : selectNormalStyles
                }
                defaultValue={_buyerIDTypes.find(
            ...
            getFieldMeta(KEY_IC_TYPE_BENEFICIARY).value,
            getFieldMeta(KEY_IC_TYPE_BENEFICIARY).error
          ])}
        </Col>
        <Col md={4} xs={12}>
          ...
          {React.useMemo(() => {
            return (
              <BaseFormGroup
              fieldName={KEY_IC_NO_BENEFICIARY}
              errors={errors}
              touched={touched}
            ...
            getFieldMeta(KEY_IC_NO_BENEFICIARY).value,
            getFieldMeta(KEY_IC_NO_BENEFICIARY).error
          ])}
