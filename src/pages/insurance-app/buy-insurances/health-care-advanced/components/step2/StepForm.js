...
import {
  ...
  intlConvertToVnd,
  isArrayEmpty, isObjEmpty
} from '../../../../../../ultity'
import {
  KEY_DURATION_SELECTED, KEY_END_INSURANCE, KEY_PACKAGE_SELECTED, KEY_PRODUCT_CODE, KEY_START_INSURANCE, KEY_YEAR,
  initialValues
} from './formikConfig'
import PopupInfoInsurance from './popupInfoInsurance'
import { DIAMOND, GOLD, PLATINUM, SLIVER, TITAN, _PACKAGE_INSURANCES, _RANGE } from './utility'
...
const StepForm = ({ stepInfo, className , packages }) => {
  ...
  const [packageShowDetail, setPackageShowDetail] = useState('SLIVER')
  const dispatch = useDispatch()
  const { step_1, step_2  ,contractId ,insuranceInfo , companyId , contractInfo , companiesData , setCompaniesData , dataFees } = stepInfo
  const totalFee = dataFees.length > 0 && dataFees.find(_elt => _elt.companyId === companyId)?.feeInsurances?.feeInsurance
  const responsive = {
    superLargeDesktop: {
  ...
  const handlePackageRadioChange = (elt_val) => {
    setFieldValue(KEY_PRODUCT_CODE, elt_val)
  }
  const togglePopup = () => {
  ...
  React.useEffect(() => {
    const currentSlide =
      getFieldMeta(KEY_PACKAGE_SELECTED).value === SLIVER
        ? 0
        : getFieldMeta(KEY_PACKAGE_SELECTED).value === TITAN
        ? 1
        : getFieldMeta(KEY_PACKAGE_SELECTED).value === GOLD
        ? 2
        : getFieldMeta(KEY_PACKAGE_SELECTED).value === PLATINUM
        ? 3
        : getFieldMeta(KEY_PACKAGE_SELECTED).value === DIAMOND
        ? 4
        : 0
  ...
  const renderCardIsMobile = () => {
    return (
      <CarouselStyled>
        ...
        >
          {!isArrayEmpty(_PACKAGE_INSURANCES) && _PACKAGE_INSURANCES.map((elt, index, _packages) => {
            const isSelected =
              getFieldMeta(KEY_PRODUCT_CODE).value === elt.value
            return (
              <PackagesBlock
                key={index}
  ...
  return (
    <div className={className}>
      ...
      <PopupInfoInsurance
        ...
        packages={_PACKAGE_INSURANCES}
        packageShowDetail={packageShowDetail}
      />

      <div className='className="px-0 px-lg-1'>
      <CardText className='mb-1 mt-1 font-weight-bold mt-2' tag='h5'>
        ...
        <CardText className='mb-1 mt-1 font-weight-bold mt-2' tag='h5'>
          <FormattedMessage id={getKeyLang(`heath-care-advanced.range`)} />
        </CardText>
        <Row xs={12} md={9}>
          <Col>
          {_RANGE.map((_elt, index) => (
            <div
              key={index}
              className='d-flex flex-cols align-items-center mb-1 cursor-pointer'
            >
              <Radio
                checked={getFieldMeta(KEY_PACKAGE_SELECTED).value === _elt.value}
                onChange={() => {
                  setFieldValue(KEY_PACKAGE_SELECTED , _elt.value)
                  }                 
                }
              />
              <span><FormattedMessage id={getKeyLang(_elt.title)} /></span>
            </div>
          ))}
          </Col>
        </Row>
      ...
      <CalFeeDone feeInsurance={intlConvertToVnd(
               totalFee,
                intl
              )} companyId={companyId} dispatch={dispatch} contractId={contractId} contractInfo={contractInfo} step_1={step_1} companiesData={companiesData} />
      {/* ============================================== */}
