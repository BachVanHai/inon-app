...
const BankTransferInfo = ({ contractCode, totalFeeInclVAT }) => {
    ...
    useEffect(() => {
        async function _getBankTransferInfo() {
            const res = await BuyInsuranceService.gettAllBanksCompanies()
            if (hasRequestFail(res && res.status)) return

            const { accountNo = " ", accountOwner = " ",
                bankName = " ", branch = " " } = res.data
            setBankTransferInfo(
                {
                    accountNo,
                    accountOwner,
    ...
    return (
      ...
      <Row>
        ...
        <Col sm="12" md="4">
        <p className="font-weight-bold text-danger font-small-3 d-flex justify-content-center">* Vui lòng ấn "
        HOÀN THÀNH " sau khi thanh toán thành công </p>
          
          <img className="mx-auto" style={{width : '100%'}} src={vietQrLink} alt="Viet QR Code"/>
        
        </Col>
        <Col sm="12"  md="8" className="margin-bottom-14 row">
          <Col md="6" sm="12" className="title-wrapper d-flex align-items-center margin-bottom-14">
