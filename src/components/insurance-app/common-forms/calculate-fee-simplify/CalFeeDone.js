...
const CalFeeDone = ({ className, dataFees, companyId, assignCompanyId, popoverFeeAction = (companyDataFee) => { } }) => {
    ...
    useEffect(() => {
        ...
        setChecked((checkState) => {
            const found = dataFees.find(elt => elt.companyName === getInsurenceCompanyMap(companyId).companyName)
            if (found) {
                dataFees.forEach(check => {
    ...
    }, [companyId , JSON.stringify(dataFees)])
