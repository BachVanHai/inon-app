...
import { DATE_FORMAT, GENDER_TYPE_FEMALE, GENDER_TYPE_MALE, GENDER_TYPE_OTHER, ID_TYPE_CCCD, ID_TYPE_CMND, ID_TYPE_HC, ID_TYPE_MST, selectErrorStyles, selectNormalStyles } from '../formik-config'
...
const InfoBase = ({
    ...
    keyMaps = {
        ...
        KEY_EMAIL: "key",
        KEY_ADDRESS: "key",
    }, className , moreField = undefined }) => {
    const intl = useIntl()
    let { sugg_gender, IDTypes } = stepInfo
    const history = useHistory()
    const { errors, touched, setFieldValue, getFieldMeta } = useFormikContext()
    const { KEY_IC_TYPE, KEY_IC_NO, KEY_FULLNAME, KEY_DATE_BIRTH, KEY_GENDER, KEY_PHONE_NUMBER, KEY_EMAIL, KEY_ADDRESS } = keyMaps
    ...
    const _checkinfoContact = async () => {
        ...
        if (hasRequestFail(res.status)) {
            BaseAppUltils.toastError(intl.formatMessage({ id: getKeyLang(`InfoNotExistsAlertContent`) }))
            return
        }
        const { phoneNumber, gender, dateOfBirth, fullName, icType, icNo, email, addresses } = res.data
        setFieldValue(KEY_PHONE_NUMBER, phoneNumber || "")
        setFieldValue(KEY_GENDER, gender || "")
        setFieldValue(KEY_DATE_BIRTH, dateOfBirth || "")
    ...
    return (
        <div className={className}>
                            ...
                            React.useMemo(() => {
                                return (
                                    <BaseFormGroup
                                        ...
                                        errors={errors}
                                        touched={touched}
                                        messageId={renderIdPersPlaceHolderAlongWithMST()}
                                    />
                                ) // eslint-disable-next-line
                            }, [getFieldMeta(KEY_IC_NO).value, getFieldMeta(KEY_IC_NO).error, getFieldMeta(KEY_IC_TYPE).value])
                        }
                        {
                            history.location.pathname === '/buy-insurance/bc' ? null :  <InputGroupAddon addonType="append">
                            <Button color="check"
                        ...
                        React.useMemo(() => {
                            ...
                            return (
                                <BaseFormDatePicker
                                    ...
                                    errors={errors}
                                    touched={touched}
                                    className={getFieldMeta(`${KEY_DATE_BIRTH}`).error && "is-invalid"}
                                    options={
                                        {
                                            maxDate: reduxName === 'buyInsuranceAntin' ? moment().utc(true).subtract(18, 'y').format(DATE_FORMAT) : moment().utc(true).subtract(1, 'd').format(DATE_FORMAT),
                                            minDate: reduxName === 'buyInsuranceHealthCare' || reduxName === 'buyInsuranceAntin' ? moment().utc(true).subtract(65, 'y').format(DATE_FORMAT) :  moment().utc(true).subtract(70, 'y').format(DATE_FORMAT),
                                            enableTime: false
                                        }
                                    }
                                />
            ...
            <Row className="mt-2">
                ...
                {
                    moreField !== undefined ? <Col md={moreField ? 4 : 6 }>{moreField} </Col> : null 
                }
